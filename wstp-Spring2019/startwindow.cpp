...
StartWindow::StartWindow(MathEngine* me, Graphs * graph) : ui(new Ui::StartWindow)
{
...
// QMessageBox to confirm if the TF has been entered correctly
    ...
    {
        // compute graph
        if(graphs->compute(numvec, dnomvec)) {
            std::cout << "**********Root Locus Coordinates**********" << endl;
            for(size_t i = 0; i < graphs->getRootLocus().size(); i++) {
                std::cout << "graph " << i << std::endl;
                std::cout << "size: " << graphs->getRootLocus()[i].size() << std::endl;
                Graphs::print(graphs->getRootLocus()[i]);
                std::cout << std::endl;
            }
            ...
            std::cout << "error computing graph" << std::endl;
        }
